...
np.save("asl_labels_new3L.npy", encoder.classes_)
...
#train model
model = Sequential([Dense(128, activation="relu", input_shape=(X.shape[1],)), Dropout(0.3), Dense(64, activation="relu"),Dropout(0.2), Dense(len(np.unique(y_encode)), activation="softmax")])
...
#save model
model.save("asl_mediapipe_model_new3L.keras")

with open("training_history_new3L.pkl", "wb") as f:
